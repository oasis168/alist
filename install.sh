...
#######################
# GitHub 相关配置
GH_DOWNLOAD_URL="${GH_PROXY}https://github.com/oasis168/alist/releases/latest/download"
#######################
...
# 获取最新版本信息的函数
get_latest_version() {
    local api_url="https://api.github.com/repos/oasis168/alist/releases/latest"
  ...
  if [ $? -eq 0 ] && [ -n "$version_info" ]; then
    ...
    case "${download_choice:-1}" in
      1)
        ...
        local official_url="https://github.com/oasis168/alist/releases/download/v$version/$filename"
      ...
      2)
        ...
        if [ -n "$proxy_input" ]; then
          ...
          GH_DOWNLOAD_URL="https://github.com/oasis168/alist/releases/latest/download"
...
INIT() {
  if [ ! -f "$INSTALL_PATH/alist" ]; then
    echo -e "\r\n${RED_COLOR}出错了${RES}，当前系统未安装 alist-pro\r\n"
    exit 1
  fi
...
UPDATE() {
    ...
    echo -e "${GREEN_COLOR}开始更新 alist-pro ...${RES}"
...
RESET_PASSWORD() {
    if [ ! -f "$INSTALL_PATH/alist" ]; then
        echo -e "\r\n${RED_COLOR}错误：系统未安装 alist-pro，请先安装！${RES}\r\n"
        exit 1
    fi
...
SHOW_MENU() {
  ...
  echo -e "\n欢迎使用 alist-pro 管理脚本\n"
  echo -e "${GREEN_COLOR}1、安装 alist-pro${RES}"
  echo -e "${GREEN_COLOR}2、更新 alist-pro${RES}"
  echo -e "${GREEN_COLOR}3、卸载 alist-pro${RES}"
...
if [ $# -eq 0 ]; then
  ...
  echo -e "用法: $0 install [安装路径]    # 安装 alist-pro"
  echo -e "     $0 update              # 更新 alist-pro"
  echo -e "     $0 uninstall          # 卸载 alist-pro"
  echo -e "     $0                    # 显示交互菜单"
fi
